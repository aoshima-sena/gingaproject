...
public class CaesarChiper {
	...
	public static String encode(String str) {
		...
		for (String strAlpha : list) {
			char charAlpha = strAlpha.charAt(0);
			int intAlpha = charAlpha;

			if (Character.isUpperCase(charAlpha)) {
				intAlpha = ((intAlpha == CODE_UPPER_Z) ? CODE_UPPER_A : intAlpha + 1);
			} else {
				intAlpha = ((intAlpha == CODE_LOWWER_Z) ? CODE_LOWWER_A : intAlpha + 1);
			}
			String convertString = Character.toString((char) intAlpha);
			encodeSb.append(convertString);
		}
	...
	public static String decode(String str) {
		...
		for (String strAlpha : list) {
			...
			String convertString = Character.toString((char) intAlpha);//int型を対応する文字列に変換

...
public class SearchInTwoDimentional {
	...
	public static void main(String[] args) {
		...
		//ここから
		//	System.out.println(twoDimentionalList.get(0).get(0));

		for (int i = 0; i < ROW; i++) {
			int max = 0;
			for (int j = 0; j < COLUMN; j++) {
				//		System.out.println(twoDimentionalList.get(i).get(j));
				//		System.out.println(twoDimentionalList.get(i).get(j - 1));
				if (max < twoDimentionalList.get(i).get(j)) {
					max = twoDimentionalList.get(i).get(j);
					//		int tmp = twoDimentionalList.get(j - 1).get(i);

					//	tmp = twoDimentionalList.get(j - 1).get(i + 1);
					//	twoDimentionalList.get(j).get(i) = tmp;
				}
				//		System.out.println(twoDimentionalList.get(j - 1).get(i));
			}
			maxNumberOfRowList.add(max);
		...
		for (int i = 0; i < ROW; i++) {
			...
			for (int j = 0; j < COLUMN; j++) {
				//		System.out.println(twoDimentionalList.get(i).get(j));
				//		System.out.println(twoDimentionalList.get(i).get(j - 1));
				if (max < twoDimentionalList.get(j).get(i)) {
					max = twoDimentionalList.get(j).get(i);
					//		int tmp = twoDimentionalList.get(j - 1).get(i);
					//	maxNumberOfColumnList.add(max);
					//	tmp = twoDimentionalList.get(j - 1).get(i + 1);
					//	twoDimentionalList.get(j).get(i) = tmp;
				}
				//		System.out.println(twoDimentionalList.get(j - 1).get(i));
		...
		int max = 0;
		for (int row = 0, column = 0; row < ROW && column < COLUMN; row++, column++) {
			if (max < twoDimentionalList.get(row).get(column)) {
				max = twoDimentionalList.get(row).get(column);
			}
		}
		maxNumberOfDiagonalList.add(max);

		max = 0;

		for (int row = 0, column = COLUMN - 1; row < ROW && column >= 0; row++, column--) {
			if (max < twoDimentionalList.get(row).get(column)) {
				max = twoDimentionalList.get(row).get(column);
			}
		}

...
public class JankenCuiGameApplicationImpl implements CuiGameApplication {
	...
	int judge() throws SystemException {
		...
		int choki = 0;
		for (int i = 0; i < playerList.size(); i++) {
			JankenPlayer JankenPlayer = playerList.get(i);
			int jnum = JankenPlayer.getJankenHand();

			switch (JankenParam.getEnum(jnum)) {
			case ROCK:
				gu++;
				break;
			case SCISSORS:
		...
		} else if (choki > 0 && pa > 0 && gu == 0) {
			return JankenParam.SCISSORS.getInt();
		} else if (pa > 0 && gu > 0 && choki == 0) {
	...
	void viewWinner() throws SystemException {
		...
		for (int i = 0; i < playerList.size(); i++) {
			JankenPlayer jankenPlayer = playerList.get(i);
			if (jankenPlayer.getJankenHand() == this.winHand) {
				win += jankenPlayer.getPlayerName() + " ";

			}
		}
		System.out.println(MessageProperties.getMessage("janken.msg.game.winner",win));
